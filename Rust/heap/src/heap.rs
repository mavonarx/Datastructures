...
impl<T: std::cmp::Ord> BinaryHeap<T> {
    pub fn new(min : bool) -> Self {
        BinaryHeap::<T>{data: Vec::<T>::new(),is_min_heap: min}
    }

    pub fn new_max () -> Self {
        BinaryHeap::<T>{data: Vec::<T>::new(),is_min_heap: false}
    }

    pub fn new_min () -> Self {
        BinaryHeap::<T>{data: Vec::<T>::new(), is_min_heap: true}
    }
    ...
    fn get_relevant_child(&self, position: usize) -> Option<usize>{
        ...
        Some(if (self.is_min_heap && self.data[left_child] < self.data[right_child])
